...
let state = getCurrentProject(); // Defined line 203: loads the active project state into memory.
...
document.querySelectorAll(".nav-button").forEach((button) => { // Browser API: attaches setup logic to every sidebar navigation button.
  button.addEventListener("click", () => showSection(button.dataset.section)); // Defined line 340: calls showSection when a nav button is clicked.
});
...
fields.forEach((field) => { // Built-in API: attaches input handlers to every planner field.
  ...
  input.addEventListener("input", () => { // Browser API: reacts when the user edits a project definition field.
    ...
    updateCurrentProject(); // Defined line 212: syncs the edited field into the active project record.
    persistProjectStore(); // Defined line 223: saves the edited project to localStorage.
    render(); // Defined line 401: refreshes generated output after the field change.
  });
});

projectSwitcher.addEventListener("change", switchProject); // Defined line 324: calls switchProject when a saved project is selected.
document.querySelector("#addFeature").addEventListener("click", addFeature); // Defined line 351: calls addFeature from the Add feature button.
document.querySelector("#addPrompt").addEventListener("click", addPrompt); // Defined line 376: calls addPrompt from the Save Prompt button.
document.querySelector("#copyReadme").addEventListener("click", copyReadme); // Defined line 605: calls copyReadme from the copy button.
document.querySelector("#savePlan").addEventListener("click", saveState); // Defined line 233: calls saveState from the footer save button.
document.querySelector("#newProject").addEventListener("click", createProject); // Defined line 261: calls createProject from the New button.
document.querySelector("#duplicateProject").addEventListener("click", duplicateProject); // Defined line 279: calls duplicateProject from the Duplicate button.
document.querySelector("#deleteProject").addEventListener("click", deleteProject); // Defined line 301: calls deleteProject from the Delete button.
document.querySelector("#resetPlan").addEventListener("click", resetPlan); // Defined line 247: calls resetPlan from the Clear Planner button.

render(); // Defined line 401: draws the initial UI from loaded state.
hasHydrated = true;
...
function createStoreFromLegacyState() { // Line 107: migrates older single-project saved data into the project collection.
  ...
  const project = {
    id: createProjectId(), // Defined line 510: assigns a unique ID to the migrated project.
...
function normalizeProjectStore(store) { // Line 131: repairs saved project collection data before the app uses it.
  const projects = store.projects.map((project) => ({ // Built-in API: normalizes every saved project record.
    id: project.id || createProjectId(), // Defined line 510: fills in missing project IDs.
...
function getCurrentProject() { // Line 203: returns the state for the active saved project.
...
function getCurrentProjectRecord() { // Line 208: returns the full active project record for metadata updates.
...
function updateCurrentProject() { // Line 212: writes the current planner state back to the active project record.
  const project = getCurrentProjectRecord(); // Defined line 208: retrieves the active project record before updating it.
  if (!project) {
    return;
  }
...
function persistProjectStore() { // Line 223: saves the project collection and legacy state copy to localStorage.
  if (!hasHydrated) {
    return;
  }
...
function saveState({ showStatus = true } = {}) { // Line 233: saves the active project and optionally shows footer feedback.
  updateCurrentProject(); // Defined line 212: writes current state into the active project before saving.
  persistProjectStore(); // Defined line 223: persists the active project and project collection.
  if (!showStatus) {
    return;
  }
...
function resetPlan() { // Line 247: clears the active planner after user confirmation.
  ...
  updateCurrentProject(); // Defined line 212: stores the cleared state in the active project record.
  persistProjectStore(); // Defined line 223: saves the cleared active project.
  syncFields(); // Defined line 333: updates the form fields after clearing.
  render(); // Defined line 401: refreshes the UI after clearing.
}

// New Project creates a blank saved project and makes it the active planner.
function createProject() { // Line 261: creates a blank saved project and switches to it.
  updateCurrentProject(); // Defined line 212: saves edits to the current project before creating another.
  const project = {
    id: createProjectId(), // Defined line 510: gives the new project a unique ID.
  ...
  state = project.state;
  syncFields(); // Defined line 333: clears the visible form fields for the new project.
  saveState(); // Defined line 233: persists the new active project.
  render(); // Defined line 401: refreshes the UI for the new project.
...
function duplicateProject() { // Line 279: copies the active project into a new saved project.
  updateCurrentProject(); // Defined line 212: saves the current project before duplicating it.
  ...
  const project = {
    id: createProjectId(), // Defined line 510: gives the duplicated project a unique ID.
  ...
  projectStore.activeProjectId = project.id;
  state = project.state;
  syncFields(); // Defined line 333: updates the form fields for the duplicated project.
  saveState(); // Defined line 233: persists the duplicated project.
  render(); // Defined line 401: refreshes the UI for the duplicated project.
}

// Delete Project removes the active saved project only after a project-specific confirmation.
function deleteProject() { // Line 301: deletes the active saved project after confirmation.
  ...
  const currentProject = getCurrentProject(); // Defined line 203: reads the active project to name it in the confirmation.
  ...
  state = getCurrentProject(); // Defined line 203: loads the next available project after deletion.
  syncFields(); // Defined line 333: updates form fields for the new active project.
  saveState(); // Defined line 233: persists the new active project selection.
  render(); // Defined line 401: refreshes the UI after deletion.
}

// The project switcher saves the current project before loading another saved project.
function switchProject() { // Line 324: switches the app to another saved project.
  updateCurrentProject(); // Defined line 212: saves the current project before switching away.
  projectStore.activeProjectId = projectSwitcher.value;
  state = getCurrentProject(); // Defined line 203: loads the selected saved project state.
  syncFields(); // Defined line 333: updates the form with the selected project.
  saveState(); // Defined line 233: persists the selected project as active.
  render(); // Defined line 401: refreshes the UI after switching projects.
}

function syncFields() { // Line 333: syncs form inputs with the current project state.
...
function showSection(sectionId) { // Line 340: displays the requested planner section and updates nav state.
...
function addFeature() { // Line 351: adds a required or future feature to the active project.
  ...
  updateCurrentProject(); // Defined line 212: stores the new feature in the active project record.
  persistProjectStore(); // Defined line 223: saves the updated feature list.
  input.value = "";
  render(); // Defined line 401: refreshes the feature lists and README output.
}

function removeFeature(index) { // Line 368: removes a feature from the active project by list index.
  state.features.splice(index, 1); // Built-in API: removes the selected feature from project state.
  updateCurrentProject(); // Defined line 212: stores the feature removal in the active project record.
  persistProjectStore(); // Defined line 223: saves the updated feature list.
  render(); // Defined line 401: refreshes the feature lists and README output.
}

// Prompt tracking captures the AI prompt, category, and lesson learned for project evidence.
function addPrompt() { // Line 376: saves an AI prompt and learning note to the active project.
  ...
  state.prompts.push({ category, prompt, learning }); // Built-in API: appends the prompt entry to project state.
  updateCurrentProject(); // Defined line 212: stores the prompt entry in the active project record.
  persistProjectStore(); // Defined line 223: saves the updated prompt history.
  ...
  render(); // Defined line 401: refreshes the prompt timeline.
}

function removePrompt(index) { // Line 393: removes a saved prompt entry by list index.
  state.prompts.splice(index, 1); // Built-in API: removes the selected prompt from project state.
  updateCurrentProject(); // Defined line 212: stores the prompt removal in the active project record.
  persistProjectStore(); // Defined line 223: saves the updated prompt history.
  render(); // Defined line 401: refreshes the prompt timeline.
}

// Rendering rebuilds all derived UI from the current state so lists, README, and score stay synced.
function render() { // Line 401: refreshes all derived UI from the current project state.
  renderProjectSwitcher(); // Defined line 411: refreshes saved-project dropdown options.
  renderFeatures(); // Defined line 424: refreshes required and future feature lists.
  renderPrompts(); // Defined line 450: refreshes the prompt timeline.
  renderChecklist(); // Defined line 481: refreshes the submission checklist.
  renderReadme(); // Defined line 518: refreshes the README draft output.
  renderReadiness(); // Defined line 523: refreshes the readiness score.
}

// Saved-project labels combine the project name with the last updated date in the dropdown.
function renderProjectSwitcher() { // Line 411: rebuilds the saved-project dropdown labels.
  ...
  projectStore.projects.forEach((project) => { // Built-in API: creates one dropdown option per saved project.
    ...
    option.textContent = getProjectLabel(project); // Defined line 501: formats the project label for the dropdown.
...
function renderFeatures() { // Line 424: rebuilds required and future feature lists safely.
  ...
  state.features.forEach((feature, index) => { // Built-in API: renders every saved feature entry.
    ...
    remove.addEventListener("click", () => removeFeature(index)); // Defined line 368: calls removeFeature for this feature row.
...
function renderPrompts() { // Line 450: rebuilds the AI prompt timeline safely.
  ...
  state.prompts.forEach((entry, index) => { // Built-in API: renders every saved prompt entry.
    ...
    item.append(category, prompt, learning); // Browser API: adds prompt details to the timeline item.

    const remove = document.createElement("button"); // Browser API: creates the prompt remove button.
    remove.className = "remove-button";
    remove.type = "button";
    remove.textContent = "Remove";
    remove.addEventListener("click", () => removePrompt(index)); // Defined line 393: calls removePrompt for this prompt row.
...
function renderChecklist() { // Line 481: rebuilds the submission checklist and checkbox handlers.
  checklist.innerHTML = "";
  ...
  checklistItems.forEach(([key, label]) => { // Built-in API: renders every checklist item.
    ...
    input.addEventListener("change", () => { // Browser API: handles checklist state changes.
      ...
      updateCurrentProject(); // Defined line 212: stores the changed checklist item in the active project.
      persistProjectStore(); // Defined line 223: saves the updated checklist.
      renderReadiness(); // Defined line 523: recalculates the score after checklist changes.
    });
...
function getProjectLabel(project) { // Line 501: creates the dropdown label for a saved project.
...
function createProjectId() { // Line 510: creates a unique ID for each saved project.
...
function renderReadme() { // Line 518: writes the generated README draft to the output panel.
  readmeOutput.textContent = buildReadme(); // Defined line 542: builds and displays the latest README draft.
}

// Readiness scoring turns completed planning fields, features, prompts, and checklist items into progress.
function renderReadiness() { // Line 523: calculates and displays the submission readiness score.
...
function buildReadme() { // Line 542: builds the Markdown README draft from planner data.
...
async function copyReadme() { // Line 605: copies the generated README draft to the clipboard.
