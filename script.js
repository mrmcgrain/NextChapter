...
const defaultState = {
  ...
  prompts: buildDefaultPromptHistory(), // Defined line 656: seeds new browsers with the project prompt history.
  checklist: {
    repo: false,
    pages: false,
    app: false,
...
let state = getCurrentProject(); // Defined line 239: loads the active project state into memory.
...
document.querySelectorAll(".nav-button").forEach((button) => { // Browser API: attaches setup logic to every sidebar navigation button.
  button.addEventListener("click", () => showSection(button.dataset.section)); // Defined line 376: calls showSection when a nav button is clicked.
});
...
fields.forEach((field) => { // Built-in API: attaches input handlers to every planner field.
  ...
  input.addEventListener("input", () => { // Browser API: reacts when the user edits a project definition field.
    ...
    updateCurrentProject(); // Defined line 248: syncs the edited field into the active project record.
    persistProjectStore(); // Defined line 259: saves the edited project to localStorage.
    render(); // Defined line 439: refreshes generated output after the field change.
  });
});

projectSwitcher.addEventListener("change", switchProject); // Defined line 360: calls switchProject when a saved project is selected.
document.querySelector("#addFeature").addEventListener("click", addFeature); // Defined line 387: calls addFeature from the Add feature button.
document.querySelector("#addPrompt").addEventListener("click", addPrompt); // Defined line 431: calls addPrompt from the Save Prompt button.
document.querySelector("#copyReadme").addEventListener("click", copyReadme); // Defined line 648: calls copyReadme from the copy button.
document.querySelector("#savePlan").addEventListener("click", saveState); // Defined line 269: calls saveState from the footer save button.
document.querySelector("#newProject").addEventListener("click", createProject); // Defined line 297: calls createProject from the New button.
document.querySelector("#duplicateProject").addEventListener("click", duplicateProject); // Defined line 315: calls duplicateProject from the Duplicate button.
document.querySelector("#deleteProject").addEventListener("click", deleteProject); // Defined line 337: calls deleteProject from the Delete button.
document.querySelector("#resetPlan").addEventListener("click", resetPlan); // Defined line 283: calls resetPlan from the Clear Planner button.

render(); // Defined line 439: draws the initial UI from loaded state.
hasHydrated = true;
...
function createStoreFromLegacyState() { // Line 107: migrates older single-project saved data into the project collection.
  ...
  const project = {
    id: createProjectId(), // Defined line 553: assigns a unique ID to the migrated project.
...
function normalizeProjectStore(store) { // Line 131: repairs saved project collection data before the app uses it.
  const projects = store.projects.map((project) => ({ // Built-in API: normalizes every saved project record.
    id: project.id || createProjectId(), // Defined line 553: fills in missing project IDs.
...
function normalizeState(rawState = {}) { // Line 145: fills missing planner fields and validates saved state shape.
  ...
  normalized.projectName = normalizeText(source.projectName); // Defined line 199: sanitizes saved project name text.
  normalized.targetUser = normalizeText(source.targetUser); // Defined line 199: sanitizes saved target user text.
  normalized.problem = normalizeText(source.problem); // Defined line 199: sanitizes saved problem text.
  normalized.value = normalizeText(source.value); // Defined line 199: sanitizes saved value text.
  normalized.solution = normalizeText(source.solution); // Defined line 199: sanitizes saved solution text.
  normalized.mvp = normalizeText(source.mvp); // Defined line 199: sanitizes saved MVP text.
  normalized.features = Array.isArray(source.features) // Built-in API: checks that saved features are an array before using them.
    ? source.features.map(normalizeFeature).filter(Boolean) // Defined line 203: validates feature entries, then filters invalid results.
  ...
  normalized.prompts = Array.isArray(source.prompts) // Built-in API: checks that saved prompts are an array before using them.
    ? source.prompts.map(normalizePrompt).filter(Boolean) // Defined line 219: validates prompt entries, then filters invalid results.
    : [];
...
function isEmptySavedState(source) { // Line 168: detects blank saved data that should receive the seeded default.
...
function createBlankProjectState() { // Line 183: returns an empty planner state for new saved projects.
  return {
...
function normalizeText(value) { // Line 199: converts non-string values into safe empty text.
  return typeof value === "string" ? value : "";
}

function normalizeFeature(feature) { // Line 203: validates a saved feature entry and its required/future type.
  if (!feature || typeof feature !== "object") {
    return null;
  }

  const text = normalizeText(feature.text).trim(); // Defined line 199: sanitizes feature text, then trims it with a built-in string method.
  if (!text) {
    return null;
  }
...
function normalizePrompt(promptEntry) { // Line 219: validates a saved prompt-history entry.
  if (!promptEntry || typeof promptEntry !== "object") {
    return null;
  }

  const prompt = normalizeText(promptEntry.prompt).trim(); // Defined line 199: sanitizes prompt text, then trims it with a built-in string method.
  const why = normalizeText(promptEntry.why).trim(); // Defined line 199: sanitizes why-this-mattered text, then trims it with a built-in string method.
  const learning = normalizeText(promptEntry.learning).trim(); // Defined line 199: sanitizes learning text, then trims it with a built-in string method.
  if (!prompt || !learning) {
    return null;
  }

  return {
    category: normalizeText(promptEntry.category) || "Planning", // Defined line 199: sanitizes prompt category with a Planning fallback.
    prompt,
    why,
    learning
  };
...
function getCurrentProject() { // Line 239: returns the state for the active saved project.
...
function getCurrentProjectRecord() { // Line 244: returns the full active project record for metadata updates.
...
function updateCurrentProject() { // Line 248: writes the current planner state back to the active project record.
  const project = getCurrentProjectRecord(); // Defined line 244: retrieves the active project record before updating it.
  if (!project) {
    return;
  }
...
function persistProjectStore() { // Line 259: saves the project collection and legacy state copy to localStorage.
  if (!hasHydrated) {
    return;
  }
...
function saveState({ showStatus = true } = {}) { // Line 269: saves the active project and optionally shows footer feedback.
  updateCurrentProject(); // Defined line 248: writes current state into the active project before saving.
  persistProjectStore(); // Defined line 259: persists the active project and project collection.
  if (!showStatus) {
    return;
  }
...
function resetPlan() { // Line 283: clears the active planner after user confirmation.
  ...
  updateCurrentProject(); // Defined line 248: stores the cleared state in the active project record.
  persistProjectStore(); // Defined line 259: saves the cleared active project.
  syncFields(); // Defined line 369: updates the form fields after clearing.
  render(); // Defined line 439: refreshes the UI after clearing.
}

// New Project creates a blank saved project and makes it the active planner.
function createProject() { // Line 297: creates a blank saved project and switches to it.
  updateCurrentProject(); // Defined line 248: saves edits to the current project before creating another.
  const project = {
    id: createProjectId(), // Defined line 553: gives the new project a unique ID.
    updatedAt: new Date().toISOString(), // Built-in API: timestamps the new blank project.
    state: createBlankProjectState() // Defined line 183: starts the new project from an empty planner state.
  };
  ...
  state = project.state;
  syncFields(); // Defined line 369: clears the visible form fields for the new project.
  saveState(); // Defined line 269: persists the new active project.
  render(); // Defined line 439: refreshes the UI for the new project.
...
function duplicateProject() { // Line 315: copies the active project into a new saved project.
  updateCurrentProject(); // Defined line 248: saves the current project before duplicating it.
  ...
  const project = {
    id: createProjectId(), // Defined line 553: gives the duplicated project a unique ID.
  ...
  projectStore.activeProjectId = project.id;
  state = project.state;
  syncFields(); // Defined line 369: updates the form fields for the duplicated project.
  saveState(); // Defined line 269: persists the duplicated project.
  render(); // Defined line 439: refreshes the UI for the duplicated project.
}

// Delete Project removes the active saved project only after a project-specific confirmation.
function deleteProject() { // Line 337: deletes the active saved project after confirmation.
  ...
  const currentProject = getCurrentProject(); // Defined line 239: reads the active project to name it in the confirmation.
  ...
  state = getCurrentProject(); // Defined line 239: loads the next available project after deletion.
  syncFields(); // Defined line 369: updates form fields for the new active project.
  saveState(); // Defined line 269: persists the new active project selection.
  render(); // Defined line 439: refreshes the UI after deletion.
}

// The project switcher saves the current project before loading another saved project.
function switchProject() { // Line 360: switches the app to another saved project.
  updateCurrentProject(); // Defined line 248: saves the current project before switching away.
  projectStore.activeProjectId = projectSwitcher.value;
  state = getCurrentProject(); // Defined line 239: loads the selected saved project state.
  syncFields(); // Defined line 369: updates the form with the selected project.
  saveState(); // Defined line 269: persists the selected project as active.
  render(); // Defined line 439: refreshes the UI after switching projects.
}

function syncFields() { // Line 369: syncs form inputs with the current project state.
...
function showSection(sectionId) { // Line 376: displays the requested planner section and updates nav state.
...
function addFeature() { // Line 387: adds a required or future feature to the active project.
  ...
  updateCurrentProject(); // Defined line 248: stores the new feature in the active project record.
  persistProjectStore(); // Defined line 259: saves the updated feature list.
  input.value = "";
  render(); // Defined line 439: refreshes the feature lists and README output.
}

function removeFeature(index) { // Line 404: removes a feature from the active project by list index.
  state.features.splice(index, 1); // Built-in API: removes the selected feature from project state.
  updateCurrentProject(); // Defined line 248: stores the feature removal in the active project record.
  persistProjectStore(); // Defined line 259: saves the updated feature list.
  render(); // Defined line 439: refreshes the feature lists and README output.
}

// Prompt tracking captures the AI prompt, category, and lesson learned for project evidence.
function addPrompt() { // Line 412: saves an AI prompt and learning note to the active project.
  ...
  updateCurrentProject(); // Defined line 248: stores the prompt entry in the active project record.
  persistProjectStore(); // Defined line 259: saves the updated prompt history.
  ...
  render(); // Defined line 439: refreshes the prompt timeline.
}

function removePrompt(index) { // Line 431: removes a saved prompt entry by list index.
  state.prompts.splice(index, 1); // Built-in API: removes the selected prompt from project state.
  updateCurrentProject(); // Defined line 248: stores the prompt removal in the active project record.
  persistProjectStore(); // Defined line 259: saves the updated prompt history.
  render(); // Defined line 439: refreshes the prompt timeline.
}

// Rendering rebuilds all derived UI from the current state so lists, README, and score stay synced.
function render() { // Line 439: refreshes all derived UI from the current project state.
  renderProjectSwitcher(); // Defined line 449: refreshes saved-project dropdown options.
  renderFeatures(); // Defined line 462: refreshes required and future feature lists.
  renderPrompts(); // Defined line 488: refreshes the prompt timeline.
  renderChecklist(); // Defined line 524: refreshes the submission checklist.
  renderReadme(); // Defined line 561: refreshes the README draft output.
  renderReadiness(); // Defined line 566: refreshes the readiness score.
}

// Saved-project labels combine the project name with the last updated date in the dropdown.
function renderProjectSwitcher() { // Line 449: rebuilds the saved-project dropdown labels.
  ...
  projectStore.projects.forEach((project) => { // Built-in API: creates one dropdown option per saved project.
    ...
    option.textContent = getProjectLabel(project); // Defined line 544: formats the project label for the dropdown.
...
function renderFeatures() { // Line 462: rebuilds required and future feature lists safely.
  ...
  state.features.forEach((feature, index) => { // Built-in API: renders every saved feature entry.
    ...
    remove.addEventListener("click", () => removeFeature(index)); // Defined line 404: calls removeFeature for this feature row.
...
function renderPrompts() { // Line 488: rebuilds the AI prompt timeline safely.
  ...
  state.prompts.forEach((entry, index) => { // Built-in API: renders every saved prompt entry.
    ...
    remove.type = "button";
    remove.textContent = "Remove";
    remove.addEventListener("click", () => removePrompt(index)); // Defined line 431: calls removePrompt for this prompt row.
...
function renderChecklist() { // Line 524: rebuilds the submission checklist and checkbox handlers.
  checklist.innerHTML = "";
  ...
  checklistItems.forEach(([key, label]) => { // Built-in API: renders every checklist item.
    ...
    input.addEventListener("change", () => { // Browser API: handles checklist state changes.
      ...
      updateCurrentProject(); // Defined line 248: stores the changed checklist item in the active project.
      persistProjectStore(); // Defined line 259: saves the updated checklist.
      renderReadiness(); // Defined line 566: recalculates the score after checklist changes.
    });
...
function getProjectLabel(project) { // Line 544: creates the dropdown label for a saved project.
...
function createProjectId() { // Line 553: creates a unique ID for each saved project.
...
function renderReadme() { // Line 561: writes the generated README draft to the output panel.
  readmeOutput.textContent = buildReadme(); // Defined line 585: builds and displays the latest README draft.
}

// Readiness scoring turns completed planning fields, features, prompts, and checklist items into progress.
function renderReadiness() { // Line 566: calculates and displays the submission readiness score.
...
function buildReadme() { // Line 585: builds the Markdown README draft from planner data.
...
async function copyReadme() { // Line 648: copies the generated README draft to the clipboard.
...
function buildDefaultPromptHistory() { // Line 656: returns the project prompt history used by fresh default state.
  return [
    {
